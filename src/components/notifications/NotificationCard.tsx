...
export function NotificationCard({ notification, onPress }: NotificationCardProps) {
  ...
  return (
    <TouchableOpacity
      ...
      className={`px-5 py-4 border-b border-border/40 flex-row items-start ${!notification.isRead ? (isDark ? 'bg-[#1C398E]/10' : 'bg-[#1C398E]/5') : 'bg-background'}`}
    >
      {/* Left Media Area */}
      <View className="mr-4 relative">
        {notification.imageUrl ? (
          <Image source={{ uri: notification.imageUrl }} className="w-14 h-14 rounded-full border border-border/50" />
        ) : (
          <View className={`w-14 h-14 rounded-full items-center justify-center border border-border/50 ${notification.avatarText ? 'bg-primary/10' : iconConfig.bg}`}>
            {notification.avatarText ? (
              <Text className="text-primary font-bold text-lg">{notification.avatarText}</Text>
            ) : (
      ...
      <View className="flex-1 justify-center">
        <Text 
          className={`text-base mb-1 ${!notification.isRead ? 'font-extrabold text-foreground' : 'font-bold text-foreground/80'}`}
          numberOfLines={2}
        >
          {notification.title}
        </Text>
